...
function IncomeDetails() {
  ...
  const fetchIncomes = async () => {
    try {
      const response = await axios.get(INCOME_URL);
      console.log('API Response:', response.data);
      
      if (response.data.success && response.data.incomes) {
        setIncomes(response.data.incomes);
        console.log('Set incomes state:', response.data.incomes);
      } else {
        setIncomes([]);
        console.log('No incomes found or API error');
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingIncome) {
        ...
        const response = await axios.put(`${INCOME_URL}/${editingIncome._id}`, incomeData);
        console.log('Update response:', response.data);
        setEditingIncome(null);
      } else {
        // Create new income
        const response = await axios.post(INCOME_URL, incomeData);
      ...
      setError(`Failed to save income details: ${err.response?.data?.message || err.message}`);
    }
  };
  ...
  const downloadPDF = () => {
    try {
      ...
      doc.setFontSize(20);
      doc.setTextColor(30, 58, 138); // Professional dark blue
      doc.text('INCOME DETAILS REPORT', pageWidth / 2, 70, { align: 'center' });
      ...
      doc.text(`Total Records: ${filteredIncomes.length}`, 15, 97);
      doc.text(`Report Period: ${filteredIncomes.length > 0 ? 'All Available Data' : 'No Data'}`, pageWidth / 2 + 10, 92);
      doc.text(`Status: ${filteredIncomes.length > 0 ? 'Complete' : 'No Data Available'}`, pageWidth / 2 + 10, 97);
      
      // Prepare table data
      const tableData = filteredIncomes.map((income) => [
        income.title || 'N/A',
        `Rs ${(income.amount || 0).toFixed(2)}`,
        income.category || 'N/A',
        new Date(income.date).toLocaleDateString() || 'N/A',
      ...
      doc.setFontSize(12);
      doc.setTextColor(30, 58, 138); // Professional dark blue
      doc.text('INCOME SUMMARY', 15, finalY + 10);
      
      doc.setFontSize(10);
      doc.setTextColor(0, 0, 0);
      doc.text(`Total Income Amount: Rs ${totalIncome.toFixed(2)}`, 15, finalY + 16);
      doc.text(`Number of Income Records: ${filteredIncomes.length}`, pageWidth / 2, finalY + 16);
      ...
      doc.setFontSize(8);
      doc.setTextColor(100, 100, 100);
      doc.text('Little Nest Daycare - Confidential Income Document', 15, footerY + 8);
      ...
      doc.save(`Little-Nest-Income-Report-${currentDate.toISOString().split('T')[0]}.pdf`);
    } catch (error) {
    ...
    try {
      const worksheet = XLSX.utils.json_to_sheet(
        filteredIncomes.map(income => ({
          'Title': income.title || 'N/A',
          'Amount': (income.amount || 0).toFixed(2),
          'Category': income.category || 'N/A',
          'Date': new Date(income.date).toLocaleDateString() || 'N/A',
    ...
    <div className="income-details-container">
      ...
      <div style={{ display: 'flex', flex: '1' }}>
        ...
        <div className="income-header">
          ...
          <p>Manage daycare income records and financial tracking</p>
        </div>
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input
                    ...
                    placeholder="Enter income title (letters only)"
                    required
                  />
                </div>
                <div className="form-group">
